...
static const float view_length=200.0f;     //模型的最大三轴长度，若大于，则缩小
...
bool MainWindow::read_Binary(QDataStream& aStream)
{
    ...
    if(i==num_mesh)
    {
        ...
        model_translate();  //设置模型变换，使其位于正中心，若模型较大，则进行缩小
...
void MainWindow::model_translate()
{
    ...
    float num_trans=view_length/max_model_size;         //缩放倍数
    if(num_trans<1)
    {
...
void MainWindow::on_doubleSpinBox_Zoom_valueChanged(double arg1)
{
    if(flag)
    {
        ...
        zoom_now=static_cast<float>(arg1);
        updata_xyz();

        ui->doubleSpinBox_H->setValue(static_cast<double>(z_now));
        ui->doubleSpinBox_X->setValue(static_cast<double>(x_now));
        ui->doubleSpinBox_Y->setValue(static_cast<double>(y_now));
        flag=true;

        if(abs(old_zoom-zoom_now)>EPSINON)
...
void MainWindow::on_doubleSpinBox_X_valueChanged(double arg1)
{
    if(flag)
    {
        flag=false;
        auto old_zoom=zoom_now;
        zoom_now=static_cast<float>(arg1)/model.size_X();
        updata_xyz();

        ui->doubleSpinBox_H->setValue(static_cast<double>(z_now));
        ui->doubleSpinBox_Y->setValue(static_cast<double>(y_now));
        ui->doubleSpinBox_Zoom->setValue(static_cast<double>(zoom_now));
        flag=true;
...
void MainWindow::on_doubleSpinBox_Y_valueChanged(double arg1)
{
    if(flag)
    {
        flag=false;
        auto old_zoom=zoom_now;
        zoom_now=static_cast<float>(arg1)/model.size_Z();
        updata_xyz();
...
void MainWindow::on_doubleSpinBox_H_valueChanged(double arg1)
{
    if(flag)
    {
        flag=false;
        auto old_zoom=zoom_now;
        zoom_now=static_cast<float>(arg1)/model.size_Y();
        updata_xyz();

        ui->doubleSpinBox_X->setValue(static_cast<double>(z_now));
        ui->doubleSpinBox_Y->setValue(static_cast<double>(y_now));
        ui->doubleSpinBox_Zoom->setValue(static_cast<double>(zoom_now));
        flag=true;
...
void MainWindow::on_save_act_triggered()
{
    ...
    //调用打开文件对话框打开一个文件
     QString aFileName=QFileDialog::getSaveFileName
             (this,"选择保存的文件",curPath,"STL文件(*.stl)");
    if (aFileName.isEmpty())
        return;

    Model model_s=ui->openGLWidget->get_all_model();
    if(saveDateSTL(aFileName,model_s))
       QMessageBox::information(this,"提示消息","成功保存文件");
    else
       QMessageBox::information(this,"提示消息","文件保存错误！");
}

bool MainWindow::saveDateSTL(QString &aFileName,Model& model_s)
{
    ...
    astream.setByteOrder(QDataStream::LittleEndian);   //设置为小端字节

    //写入头文件
    string head_str="STLB ATF";
    for(int i=0;i!=72;++i)
         head_str+=' ';
    ...
    //写入三角面数量
    qint32 num_mesh=model_s.size();
    astream.writeRawData((char*)&num_mesh,4);

    auto vertexs=model_s.vertexs_of_model();
    qint16 last_stl=0;
    float x,y,z;
    float x1,y1,z1;
    ...
    {
        //写入法向量
        auto normal_1=vertexs[i*3].normal();
        x1=normal_1.x();y1=normal_1.y();z1=normal_1.z();
        astream.writeRawData( (char*) &x1, 4 );
        astream.writeRawData( (char*) &y1, 4 );
        astream.writeRawData( (char*) &z1, 4 );

        //循环写入三个顶点
        for(int j=0;j!=3;++j)
        {
            auto vertexs_1=vertexs[i*3+j].position();
            x=vertexs_1.x();y=vertexs_1.y();z=vertexs_1.z();
            astream.writeRawData( (char*) &x, 4 );
            astream.writeRawData( (char*) &y, 4 );
            astream.writeRawData( (char*) &z, 4 );
        }
        astream.writeRawData((char*)&last_stl,2);
   }
